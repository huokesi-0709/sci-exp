...
> **当前状态入口（2026-08-21）**：任何实验规划或智能体接手前，先读取
> [`CURRENT_EXPERIMENT_STATE.md`](CURRENT_EXPERIMENT_STATE.md) 和
> [`docs/实验状态总览_v1.0.yaml`](docs/实验状态总览_v1.0.yaml)。E0已完成长时采样、
> 空闲基线、同步和标记RTT，但仍等待三点及冷/热校准；仓库中的大型JSONL已由用户
> 主动截为首尾摘录，正式复核需使用外部完整副本或新session重采。正式E1–E8物理
> 能耗实验当前不得越级运行。
> 大型遥测数据的Git/外部归档边界见
> [`docs/实验数据保留与Git归档策略_v1.0.md`](docs/实验数据保留与Git归档策略_v1.0.md)。
